...
class TestInstanceCRUDAdvanced:
    ...
    def test_create_with_integrity_error_handling(self, mock_session):
        ...
        with pytest.raises(
            ValidationError, match="Instance with issue_id 'duplicate' already exists"
        ):
    ...
    def test_create_with_none_extra_metadata(self, db_session):
        ...
        instance = InstanceCRUD.create(
            session=db_session, issue_id="test-none", extra_metadata=None
        )
...
class TestTaskCRUDAdvanced:
    ...
    def test_create_with_whitespace_trimming(self, db_session):
        ...
        task = TaskCRUD.create(session=db_session, title="  Test Task Title  ")
    ...
    def test_list_by_instance_priority_ordering_edge_cases(self, db_session):
        ...
        ordered_tasks = TaskCRUD.list_by_instance(
            session=db_session, instance_id=instance.id
        )
    ...
    def test_list_pending_priority_ordering_edge_cases(self, db_session):
        ...
        for i, priority in enumerate(priorities):
            TaskCRUD.create(session=db_session, title=f"Task {i}", priority=priority)
        db_session.commit()
    ...
    def test_update_status_timezone_aware_handling(self, db_session):
        ...
        with patch("cc_orchestrator.database.crud.datetime") as mock_datetime:
            ...
            end_time = datetime(2023, 1, 1, 11, 30, 0, tzinfo=UTC)
            ...
            updated_task = TaskCRUD.update_status(
    ...
    def test_update_status_failed_and_cancelled_statuses(self, db_session):
        ...
        TaskCRUD.update_status(
            session=db_session, task_id=task2.id, status=TaskStatus.IN_PROGRESS
        )

        # Set to FAILED and CANCELLED
        failed_task = TaskCRUD.update_status(
            session=db_session, task_id=task1.id, status=TaskStatus.FAILED
        )
        cancelled_task = TaskCRUD.update_status(
            session=db_session, task_id=task2.id, status=TaskStatus.CANCELLED
        )
    ...
    def test_update_with_datetime_import_coverage(self, db_session):
        ...
        updated = TaskCRUD.update(
            session=db_session, task_id=task.id, description="Updated description"
        )
...
class TestWorktreeCRUDAdvanced:
    ...
    def test_create_with_integrity_error(self, mock_session):
        ...
        with pytest.raises(
            ValidationError, match="Worktree with path '/duplicate' already exists"
        ):
            WorktreeCRUD.create(
                session=mock_session, name="test", path="/duplicate", branch_name="main"
            )
    ...
    def test_list_by_status(self, db_session):
        ...
        active_wt = WorktreeCRUD.create(
            session=db_session, name="active", path="/active", branch_name="main"
        )
        inactive_wt = WorktreeCRUD.create(
            session=db_session, name="inactive", path="/inactive", branch_name="main"
        )
        ...
        inactive_worktrees = WorktreeCRUD.list_by_status(
            session=db_session, status=WorktreeStatus.INACTIVE
        )
    ...
    def test_update_status_with_optional_params(self, db_session):
        ...
        worktree = WorktreeCRUD.create(
            session=db_session, name="test", path="/test", branch_name="main"
        )
        db_session.commit()
    ...
    def test_update_status_datetime_import_coverage(self, db_session):
        ...
        updated = WorktreeCRUD.update_status(
            session=db_session, worktree_id=worktree.id, status=WorktreeStatus.ACTIVE
        )
    ...
    def test_delete_worktree(self, db_session):
        ...
        worktree = WorktreeCRUD.create(
            session=db_session, name="deletable", path="/deletable", branch_name="main"
        )
        db_session.commit()
...
class TestHealthCheckCRUD:
    ...
    def test_list_by_instance(self, db_session):
        ...
        assert (
            checks[0].check_timestamp
            >= checks[1].check_timestamp
            >= checks[2].check_timestamp
        )
    ...
    def test_list_by_instance_with_pagination(self, db_session):
        ...
        # Test limit
        checks = HealthCheckCRUD.list_by_instance(
            session=db_session, instance_id=instance.id, limit=2
        )
        assert len(checks) == 2

        # Test offset and limit
        checks_page2 = HealthCheckCRUD.list_by_instance(
            session=db_session, instance_id=instance.id, limit=2, offset=2
        )
    ...
    def test_list_by_instance_with_no_offset(self, db_session):
        ...
        # Test with offset=0 (should not apply offset)
        checks = HealthCheckCRUD.list_by_instance(
            session=db_session, instance_id=instance.id, offset=0
        )
        assert len(checks) == 1
    ...
    def test_count_by_instance(self, db_session):
        ...
        count1 = HealthCheckCRUD.count_by_instance(
            session=db_session, instance_id=instance1.id
        )
        count2 = HealthCheckCRUD.count_by_instance(
            session=db_session, instance_id=instance2.id
        )
...
class TestConfigurationCRUDAdvanced:
    ...
    def test_create_with_whitespace_trimming(self, db_session):
        ...
        config = ConfigurationCRUD.create(
            session=db_session, key="  test.key  ", value="test_value"
        )
    ...
    def test_create_with_none_metadata(self, db_session):
        ...
        config = ConfigurationCRUD.create(
            session=db_session, key="test.key", value="test_value", extra_metadata=None
        )
    ...
    def test_get_value_scope_hierarchy_all_scopes(self, db_session):
        ...
        value = ConfigurationCRUD.get_value(
            session=db_session, key="test.setting", scope=ConfigScope.PROJECT
        )
    ...
    def test_get_exact_by_key_scope_non_instance_scope(self, db_session):
        ...
        found_config = ConfigurationCRUD.get_exact_by_key_scope(
            session=db_session, key="global.setting", scope=ConfigScope.GLOBAL
        )
    ...
    def test_get_exact_by_key_scope_not_found(self, db_session):
        ...
        result = ConfigurationCRUD.get_exact_by_key_scope(
            session=db_session, key="nonexistent.key", scope=ConfigScope.GLOBAL
        )
...
class TestTransactionHandlingAndEdgeCases:
    ...
    def test_complex_json_metadata_handling(self, db_session):
        ...
        instance = InstanceCRUD.create(
            session=db_session, issue_id="json-test", extra_metadata=complex_metadata
        )
        db_session.commit()
...
class TestBulkOperationsAndBatchProcessing:
    ...
    def test_bulk_task_status_updates(self, db_session):
        ...
        for i in range(5):
            task = TaskCRUD.create(
                session=db_session, title=f"Bulk Task {i}", instance_id=instance.id
            )
            tasks.append(task)
        db_session.commit()
    ...
    def test_cascading_deletes_with_relationships(self, db_session):
        ...
        task1 = TaskCRUD.create(
            session=db_session, title="Related Task 1", instance_id=instance.id
        )
        task2 = TaskCRUD.create(
            session=db_session, title="Related Task 2", instance_id=instance.id
        )
        db_session.commit()
...
class TestExceptionHandlingAndErrorPaths:
    ...
    def test_worktree_status_all_values(self, db_session):
        ...
        for i, status in enumerate(WorktreeStatus):
            ...
            WorktreeCRUD.update_status(
                session=db_session, worktree_id=worktree.id, status=status
            )
