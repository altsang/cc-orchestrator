...
def demonstrate_contextual_logging():
    ...
    logger.info("Starting instance with task assignment", 
                task_title="Demo Task",
                priority="high")
    
    logger.debug("Instance health check passed",
                 cpu_usage=45.2,
                 memory_usage=128.5)
...
def demonstrate_error_handling():
    ...
    def potentially_failing_function():
        ...
        if random.choice([True, False]):
            raise InstanceError("Simulated instance failure", 
                              context={"instance_id": "claude-demo-001"})
        return "success"
...
def demonstrate_orchestrator_operations():
    ...
    log_instance_lifecycle(instance_id, "create", "success", 
                          {"worktree": "/tmp/worktree-002"})
...
def demonstrate_exception_types():
    ...
    exceptions = [
        InstanceError("Instance failed to start", 
                     context={"instance_id": "claude-001", "port": 8080}),
        TaskError("Task validation failed", 
    ...
    for exc in exceptions:
        logger.error(f"Exception demo: {exc.message}", 
                    exception=exc,
