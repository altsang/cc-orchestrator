...
class TestDatabaseManager:
    ...
    def test_create_tables(self, db_manager):
        ...
        actual_tables = {table for table, exists in validation.items() if exists and table in expected_tables}
    ...
    def test_session_context_manager(self, db_manager):
        ...
        with db_manager.get_session() as session:
            retrieved = InstanceCRUD.get_by_issue_id(session=session, issue_id="test-123")
...
class TestCompleteWorkflow:
    ...
    def test_instance_with_tasks_and_worktree(self, db_manager):
        ...
        with db_manager.get_session() as session:
            retrieved_instance = InstanceCRUD.get_by_issue_id(session=session, issue_id="workflow-test")
            ...
            instance_tasks = TaskCRUD.list_by_instance(session=session, instance_id=retrieved_instance.id)
    ...
    def test_task_status_workflow(self, db_manager):
        ...
        with db_manager.get_session() as session:
            ...
            # Start task
            TaskCRUD.update_status(session=session, task_id=task.id, status=TaskStatus.IN_PROGRESS)
            assert task.status == TaskStatus.IN_PROGRESS
            assert task.started_at is not None
            
            # Complete task
            TaskCRUD.update_status(session=session, task_id=task.id, status=TaskStatus.COMPLETED)
    ...
    def test_cascade_deletion(self, db_manager):
        ...
        with db_manager.get_session() as session:
            ...
            # Verify data exists
            assert len(TaskCRUD.list_by_instance(session=session, instance_id=instance.id)) == 2
        ...
        with db_manager.get_session() as session:
            ...
            retrieved_worktree = WorktreeCRUD.get_by_path(session=session, path="/tmp/cascade-test")
