...
class TestHealthRouterFunctions:
    ...
    def mock_crud(self):
        ...
        mock_health_check.check_timestamp = datetime.now(timezone.utc)
        mock_health_check.duration_ms = 150.5
        mock_health_check.created_at = datetime.now(timezone.utc)
        mock_health_check.updated_at = datetime.now(timezone.utc)
