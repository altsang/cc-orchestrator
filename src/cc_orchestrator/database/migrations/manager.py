...
class MigrationManager:
    ...
    def __init__(self, engine: Engine, migrations_dir: Optional[Path] = None) -> None:
    ...
    def get_applied_migrations(self) -> List[MigrationRecord]:
        ...
        with Session(self.engine) as session:
            result = session.execute(
                text("SELECT version, description, applied_at, checksum "
    ...
    def _load_migration_from_file(self, migration_file: Path) -> Optional[Migration]:
    ...
    def migrate_down(self, target_version: str) -> bool:
        ...
        for record in to_rollback:
            ...
            if migration is None:
                print(f"Warning: Migration file for version {record.version} not found.")
                continue
