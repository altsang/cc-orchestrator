...
def log_instance_lifecycle(
    ...
    if status == "success":
        logger.info(
            f"Instance {action} completed",
            action=action,
            details=details or {}
        )
    elif status == "error":
        logger.error(
            f"Instance {action} failed",
            action=action,
            details=details or {}
        )
    else:
...
def log_task_assignment(task_id: str, instance_id: str, task_details: Dict[str, Any]) -> None:
