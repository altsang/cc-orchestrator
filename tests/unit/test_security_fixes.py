...
from cc_orchestrator.web.auth import authenticate_user
...
class TestJWTSecretValidation:
    ...
    def test_jwt_secret_validation_weak_default(self):
        ...
        with patch.dict(os.environ, {"JWT_SECRET_KEY": "dev-secret-key-change-in-production"}):
            with pytest.raises(ValueError, match="JWT_SECRET_KEY must be set"):
                import importlib
                import cc_orchestrator.web.auth
                importlib.reload(cc_orchestrator.web.auth)
    ...
    def test_jwt_secret_validation_strong_key(self):
        ...
        with patch.dict(os.environ, {"JWT_SECRET_KEY": "very-strong-secret-key-for-production-use-123"}):
            try:
                import importlib
                import cc_orchestrator.web.auth
                importlib.reload(cc_orchestrator.web.auth)
...
class TestDemoUserSecurity:
    ...
    def test_demo_users_explicitly_disabled(self):
        ...
        with patch.dict(os.environ, {}):  # No ENABLE_DEMO_USERS set (defaults to false)
            with pytest.raises(HTTPException) as exc_info:
                authenticate_user("admin", "admin123")
    ...
    def test_demo_users_can_be_enabled_in_development(self):
        ...
        with patch.dict(os.environ, {
            ...
            "DEMO_ADMIN_PASSWORD": "custom-dev-password"
        }):
            ...
            user = cc_orchestrator.web.auth.authenticate_user("admin", "custom-dev-password")
...
class TestProductionEnvironmentValidation:
    ...
    def test_production_requires_frontend_url(self):
        ...
        with patch.dict(os.environ, {"DEBUG": "false"}, clear=True):
            with pytest.raises(ValueError, match="FRONTEND_URL must be set when DEBUG=false"):
    ...
    def test_production_requires_valid_frontend_url(self):
        ...
        for invalid_url in test_cases:
            with patch.dict(os.environ, {"DEBUG": "false", "FRONTEND_URL": invalid_url}):
        ...
        for valid_url in valid_urls:
            with patch.dict(os.environ, {
                ...
                try:
                    ...
                    pytest.fail(f"Valid URL {valid_url} should be accepted, but got: {e}")
    ...
    def test_development_mode_works_without_frontend_url(self):
        ...
        with patch.dict(os.environ, {
            "DEBUG": "true",
            "JWT_SECRET_KEY": "test-secret-for-dev"
        }):
            try:
                from cc_orchestrator.web.app import create_app
                app = create_app()
...
class TestSecurityBestPractices:
    ...
    def test_no_hardcoded_secrets_in_production_code(self):
        ...
        for pattern in forbidden_patterns:
            if pattern in source:
                ...
                assert "DEMO" in source or "demo" in source, f"Hardcoded {pattern} found outside demo context"
    ...
    def test_environment_variable_security(self):
        ...
        with patch.dict(os.environ, {
            ...
            user = cc_orchestrator.web.auth.authenticate_user("admin", "custom-secure-password")
            assert user is not None
