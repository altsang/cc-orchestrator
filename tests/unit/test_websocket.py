...
class TestWebSocketMessaging:
    ...
    def test_subscription_handling(self, mock_ws_manager, client, auth_token):
        ...
        with client.websocket_connect("/ws/dashboard") as websocket:
            ...
            sub_message = {"type": "subscribe", "events": ["instance_status", "system_events"]}
            ...
            mock_ws_manager.add_subscription.assert_called_with(
                "test-connection-id",
                ["instance_status", "system_events"]
            )
            
            # Should send confirmation
            mock_ws_manager.send_to_connection.assert_called_with(
                "test-connection-id",
                {"type": "subscription_confirmed", "events": ["instance_status", "system_events"]}
            )
